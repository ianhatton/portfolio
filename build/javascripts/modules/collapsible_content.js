...
class CollapsibleContentClass {
  ...
  _setBodyAriaHidden(toggle) {
    let body = this.bodies[_.indexOf(this.toggles, toggle)];
  ...
  _setItemClass(toggle) {
    let item = this.items[_.indexOf(this.toggles, toggle)];
    let className = item.className;

    if(_.includes(className, "open")) {
      item.className = className.replace(/(?:^|\s)open(?!\S)/g, "");
    } else {
      item.className = className += " open";
    }
  }
